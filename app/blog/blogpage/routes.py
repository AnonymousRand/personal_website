...
from flask import current_app, jsonify, render_template, redirect, request, send_from_directory, \
        url_for
from flask_login import current_user

import app.blog.blogpage.markdown_config as markdown_config
...
def get_posts(*args, **kwargs):
    # this should automatically redirect non-integer values to page 1
    ...
    if blogpage.is_all_posts:
        posts = db.paginate(
            (
                db.session.query(Post).join(Post.blogpage)
                .filter_by(is_login_required=False, is_published=True)
    ...
    next_page_url = url_for(f"blog.{blogpage_id}.get_posts", page=posts.next_num, _external=True) \
            if posts.has_next else None
    prev_page_url = url_for(f"blog.{blogpage_id}.get_posts", page=posts.prev_num, _external=True) \
...
# hence the lack of `@require_login_if_restricted_bp()`. however, if they are being accessed
# by a public user, we require they provide the full sanitized title in the link to prevent
# brute-force enumeration of post IDs and make being unlisted at least somewhat meaningful
...
def get_post(post, post_id, post_sanitized_title, *args, **kwargs):
    ...
    content_md = None
    if post.content:
        content_md = markdown.Markdown(extensions=markdown_config.POST_EXTS)
...
def get_comments(post, post_id, *args, **kwargs):
    def sanitize_comment_html(s: str) -> str:
        ...
        # somewhat active and there doesn't seem to be great alternatives at the moment
        s = bleach.clean(
            s,
            tags=[
                "abbr", "acronym", "b", "blockquote", "br", "center", "code", "details", "div",
                "em", "h1", "h2", "h3", "i", "li", "p", "pre", "ol", "small", "span", "strong",
                "sub", "summary", "sup", "table", "tbody", "td", "th", "thead", "tr", "ul"
            ],
            attributes=[
                "class", "colspan", "data-align-bottom", "data-align-center", "data-align-right",
    ...
    for comment in comments:
        if comment.author == current_app.config["VERIFIED_AUTHOR"]:
            comment.content = markdown.markdown(
                comment.content, extensions=markdown_config.COMMENT_EXTS_VERIFIED
            )
        else:
            comment.content = markdown.markdown(
                comment.content, extensions=markdown_config.COMMENT_EXTS
            )
            comment.content = sanitize_comment_html(comment.content)
 
    add_comment_form = AddCommentForm()
    return jsonify(html=render_template(
        "blog/blogpage/post_comments.html", post=post, comments=comments,
        add_comment_form=add_comment_form
    ))
...
def add_comment(post, post_id, *args, **kwargs):
    ...
    comment = Comment(
        author=author, content=request.form.get("content"), post=post,
        is_unread=(not is_verified_author)
    )
...
def edit_comment_form(post, post_id, comment, comment_id, *args, **kwargs):
    # pre-populate form fields with existing comment content; leave out `parent` since that
    # should not be changeable
    form = EditCommentForm(obj=comment)
    return jsonify(html=render_template(
        "blog/blogpage/post_comment_edit_form.html", form=form, comment=comment
    ))
...
def get_file(post, post_id, file_name, *args, **kwargs):
    return send_from_directory(
        f"{current_app.config['ROOT_TO_BLOGPAGE_STATIC']}/{post.blogpage_id}/files/{post_id}",
        file_name
    )
